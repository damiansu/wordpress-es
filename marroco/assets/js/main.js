...
var App = {
	...
	bind: function() {
        ...
        // mobile megamenu fix
        if( jQuery('body').hasClass('bw-is-mobile') ) {
            jQuery(document).on('click', '#header #navigation > ul > li > a', function(e) {
                if( jQuery(this).closest('li').find('.bw-megamenu:not(.open)').length ) {
                    e.preventDefault();
                }
            });
        }
